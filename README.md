...
(ii)if it is a goal state, then return it and quit
(iii)if it is not a goal state but better than current state then make new state as current state
(iv)if it is not better than current state then continue in the loop
``` 
## Steps Applied:
## Step-1
